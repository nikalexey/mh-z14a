...
void MHZ::setAutoCalibrate(
    boolean b)  // only available for MHZ-19B with firmware < 1.6 and MHZ 14a
{
  uint8_t cmd_enableAutoCal[9] = {0xFF, 0x01, 0x79, 0xA0, 0x00,
                                  0x00, 0x00, 0x00, 0xE6};
  uint8_t cmd_disableAutoCal[9] = {0xFF, 0x01, 0x79, 0x00, 0x00,
                                   0x00, 0x00, 0x00, 0x86};
  if (b) {
...
/***** calibrateSpan() function for professional use. requires a constant
atmosphere with 2K, 5k or 10k ppm CO2 and calibrateZero at first.
